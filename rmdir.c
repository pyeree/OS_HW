...
// ── 재귀: subtree(children·siblings) 전체 메모리 해제
...
// ── 빈 디렉터리인지 확인 (children 가 NULL 이면 비어 있음)
int is_empty_directory(TreeNode* dir) {
    return dir->children == NULL;
}

// ── parent->children 리스트에서 target 노드를 잘라내고 해제
void remove_child(TreeNode* parent, TreeNode* target) {
    TreeNode *curr = parent->children, *prev = NULL;
    while (curr) {
        if (curr == target) {
            if (prev) prev->next_sibling = curr->next_sibling;
            else        parent->children   = curr->next_sibling;
            free_subtree(curr);
            return;
        }
        prev = curr;
...
TreeNode* find_child_dir(TreeNode* parent, const char* name) {
    for (TreeNode* c = parent->children; c; c = c->next_sibling) {
...
int remove_dir_path(DirectoryTree* dTree, const char* path) {
    ...
    if (cnt==0) { printf("rmdir: invalid path\n"); return -1; }
